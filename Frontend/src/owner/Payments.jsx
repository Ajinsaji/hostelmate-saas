...
export default function Payments() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 4. Payment Cards (Mobile) & Table (Desktop) */}
      {loading ? (
        ...
        <>
          ...
          <div className="lg:hidden space-y-3">
            {filteredPayments.map((p) => {
              const { paid, balance, status } = calcTotals(p);
              const resName = p.residentId?.name || "Resident";
              return (
                <MobileCard
                  key={p._id}
                  avatar={<Avatar name={resName} />}
                  title={resName}
                  subtitle={`Room ${p.residentId?.roomId?.roomNumber || '—'} • ${p.month || 'Current Month'}`}
                  badge={
                    <Badge variant={status === "paid" ? "success" : "danger"}>
                      ₹{paid.toLocaleString()}
                    </Badge>
                  }
                  onMenuClick={() => deletePayment(p._id)}
                >
                  <div className="flex justify-between text-xs pt-1">
                    <span style={{ color: colors.text.secondary || "#94A3B8" }}>Method: {p.method || 'UPI'}</span>
                    <span style={{ color: balance === 0 ? colors.accent.success : colors.accent.danger, fontWeight: typography.weights.bold }}>
                      {balance === 0 ? "Paid In Full" : `Due: ₹${balance.toLocaleString()}`}
                    </span>
                  </div>
                </MobileCard>
              );
