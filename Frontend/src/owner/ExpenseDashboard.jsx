...
export function ExpenseDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 4. Timeline Cards (Mobile) & Table (Desktop) */}
      {loading ? (
          ...
          <div className="lg:hidden space-y-3">
            {filteredExpenses.map((exp) => (
              <MobileCard
                key={exp._id}
                title={exp.title}
                subtitle={`${exp.categoryId?.categoryName || 'General'} • ${new Date(exp.expenseDate || Date.now()).toLocaleDateString()}`}
                badge={<Badge variant="danger">₹{(exp.netAmount || exp.amount || 0).toLocaleString()}</Badge>}
                onMenuClick={() => handleDeleteExpense(exp._id)}
              >
                <div className="flex justify-between text-xs pt-1">
                  <span style={{ color: colors.text.secondary || "#94A3B8" }}>Vendor: {exp.vendorId?.vendorName || "Direct"}</span>
                  <Badge variant={exp.status === "Paid" ? "success" : "warning"} size="sm">{exp.status || "Paid"}</Badge>
                </div>
              </MobileCard>
            ))}
          </div>
