...
export default function PublicHostel() {
  ...
  if (loading) {
    return <PageContainer title="Loading..." subtitle="Fetching hostel details" />;
  }

  if (!hostel) {
    return (
      <PageContainer title="Not Found" subtitle="This hostel does not exist">
        <Card>
          <p className="text-slate-300">The requested hostel could not be found.</p>
          <button onClick={() => navigate('/')} className="mt-4 px-4 py-2 rounded-lg" style={{ background: colors.accent.primary, color: 'white' }}>
            Go Home
          </button>
        </Card>
      </PageContainer>
  ...
  return (
    <PageContainer title={hostel.hostelName} subtitle="Welcome to our hostel">
      <Card className="mb-6">
      ...
      <Card className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex items-center gap-2 text-slate-300">
            <CheckCircle size={16} style={{ color: colors.accent.primary }} />
      ...
      <div className="flex justify-center mt-8">
        <button 
          ...
          style={{ background: colors.accent.primary, color: 'white', boxShadow: `0 4px 20px ${colors.accent.primary}40` }}
        >
